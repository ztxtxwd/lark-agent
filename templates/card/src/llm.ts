...
上下文说明：
- 默认不预拉历史。若判定用户在改已有卡片，会注入该卡的**完整 schema 2.0 JSON**、body.elements 索引和底稿路径（会话里最晚一张卡，不限谁发的）。完整 DSL 已在上下文里，**不要**再调 get_message_detail。
- 判定：用户话里有改/修改/换成/这张卡等字眼；或本轮是转发的卡片（interactive / 合并转发）；或用户用飞书「回复」了某条消息。
